...
if($action == "userdel") {
  ...
  mysql_select_db("mysql",$link) or die( __("Unable to use mysql database") );
...
<input type=submit value=Go onclick="if(!document.search.search.value) { alert(\''. __('Please enter in a search value!') .'\'); return false; }">';
  ...
  if($_GET[search]) print ' <input type=button value="'. __('Show All') .'" onclick="self.location=\'databases.php\'">';
  ...
  if($num == 0 and !$_GET[search]) print __('There are no databases setup');
  else if($_GET[search]) print __('Your search returned') .' <i><b>' . $num . '</b></i> '. __('results') .'<p>';

  if($num != 0) print '<table><tr><th>'. __('Domain') .'</th><th>'. __('Database') .'</th></tr>';
