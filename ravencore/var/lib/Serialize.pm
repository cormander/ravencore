...
Unserializing NULL value (i:1;)
corrected to:
Unserializing integer or double (i:1;)

2) It didn't compensate for strings with newline characters, so multi-line
strings being unserialized came out as an empty string

Lines of code that have been changed are marked with: BUGFIX by cormander
...
sub unserialize_value {
	...
	# BUGFIX by cormander, multi-line strings were comming back as empty
	elsif ($value =~ /^s:(\d+):"(.*)";$/s) {
	...
	# BUGFIX by cormander, single digit integers were combing back as NULL
	elsif ($value =~ /^(i|d):(\-?\d+(\.?\d+)?);$/) {
