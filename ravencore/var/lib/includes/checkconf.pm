...
sub rebuild_conf_file {
	...
	#
	# append a header to each conf file, saying that it was created by ravencore, and that
	# if you wan't to edit it, create a .local file and edit that instead
	#

	$file_data = '########################################################################################################
#
# This file was generated by a RavenCore Control Panel Process
# If you make a change to this file, your change will be overwritten!!
#
# To make a change to this file and have it stick around, create a .local file and edit it instead,
# and restart ravencore. The appropriate daemon should be restarted for you.
#
# Here is an example
# cd /usr/local/ravencore/etc/postfix/main.cf; cp mynetworks mynetworks.local; vi mynetworks.local
# and edit it as you like, then restart ravencore
#
# Other examples:
# cd /usr/local/ravencore/etc/vsftpd/vsftpd.conf; cp ftpd_banner ftpd_banner.local; vi ftpd_banner.local
# cd /usr/local/ravencore/etc/; cp amavisd.conf.in amavisd.conf.in.local; vi amavisd.conf.in.local
#
########################################################################################################

' . $file_data;

	# rewrite the file if $file_data differs from what's there
